...
async def test_temperature_range_command_sends_range():
    """The portal's range dialog posts the chosen range as range."""
    session = FakeSession(post_responses=[login_response(), command_response()])
    client = ControlMySpaClient(session, "user@example.com", "secret")

    await client.async_set_temperature_range("spa-1", "LOW")

    url, kwargs = session.post_calls[1]
    assert url.endswith("/web/spa-commands/temperature/range")
